...
if (isset($_POST['reg_user'])) {
  ...
  if ($user) { // if user exists
    if ($user['username'] === $username) {
      array_push($errors, "Данное имя пользователя уже зарегестрировано");
    }

    if ($user['email'] === $email) {
      array_push($errors, "Данная почта уже зарегистрирована");
    }
  }
  ...
  if (count($errors) == 0) {
  	...
  	if (mysqli_num_rows($results) == 1) {
  	  ...
  	  header('location: index.php');
  		...
  		array_push($errors, "Неверные логин или пароль");
  	}
  }
}
